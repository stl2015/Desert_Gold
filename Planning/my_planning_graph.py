...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        :param symbol: expr == str ? 
        :param is_pos: bool - is pos or neg literal.
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # loop over actions universe 
        # it is connected to the S node in previous s_level set, by setting the parents - child links.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for p1 in node_a1.action.effect_add:
            if p1 in node_a2.action.effect_rem: return True
        
        for p2 in node_a2.action.effect_add:
            if p2 in node_a1.action.effect_rem: return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for p1 in node_a1.action.effect_add:
            if p1 in node_a2.action.precond_neg: return True
        
        for p2 in node_a2.action.effect_add:
            if p2 in node_a1.action.precond_neg: return True
            
        for n1 in node_a1.action.effect_rem:
            if n1 in node_a2.action.precond_pos: return True            
            
        for n2 in node_a2.action.effect_rem:
            if n2 in node_a1.action.precond_pos: return True  
...
#        for n1 in node_a1.action.precond_neg:
#            if n1 in node_a2.action.precond_pos: return True            

        # check if their parents are mutex
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos: return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all(a1.is_mutex(a2) for a1 in p1 for a2 in p2)
    ...
    def h_levelsum(self) -> int:
        ...
        gg = list()
        gg[:] = self.problem.goal
        for l in range(len(self.s_levels)):
            for s in self.s_levels[l]:
                if s.symbol in gg and s.is_pos == True: 
                    level_sum+=l
                    gg.remove(s.symbol)
                if len(gg) == 0: return level_sum
